...
class admin extends ecjia_admin {	
	private $db_friendlink;
	
	public function __construct() {
		parent::__construct();
		$this->db_friendlink = RC_Loader::load_app_model('friendlink_model');
	 ...
	 */
	public function add(){
		$this->admin_priv('friendlink_add');
	...
	public function insert() {
		...
		/* 查看名称是否有重复 */
		$query = $this->db_friendlink->friendlink_count(array('link_name' => $link_name));
		if (isset($_POST['link_name'])){
			if ($query > 0) {
				$this->showmessage(RC_Lang::get('friendlink::friend_link.link_name_exist'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		}
		...
		$id = $this->db_friendlink->friendlink_manage($data);
		...
		$this->showmessage(RC_Lang::get('friendlink::friend_link.add_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('friendlink/admin/edit', array('id' => $id))));
	}
	...
	public function edit() {
		...
		$link_arr = $this->db_friendlink->friendlink_find(array('link_id' => intval($_GET['id'])));
		/* 标记为图片链接还是文字链接 */
	...
	public function update() {
		...
		$row = $this->db_friendlink->friendlink_find(array('link_id' => $id), 'link_name, link_logo');
		if ($link_name != $row['link_name']) {
			if ($this->db_friendlink->friendlink_count(array('link_name' => $link_name)) != 0) {
				$this->showmessage(RC_Lang::get('friendlink::friend_link.link_name_exist'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		}	
		...
		if ((isset($_FILES['link_img']['error']) && $_FILES['link_img']['error'] == 0) || (!isset($_FILES['link_img']['error']) && isset($_FILES['link_img']['tmp_name']) && $_FILES['link_img']['tmp_name'] != 'none')) {
			...
			if (!empty($image_info)) {
				if ((strpos($row['link_logo'], 'http://') === false) && (strpos($row['link_logo'], 'https://') === false)) {
					$img_name = $row['link_logo'];
					$upload->remove($img_name);
				}
				$link_logo = $upload->get_position($image_info);
			} else {
				$this->showmessage($upload->error(), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		...
		$this->db_friendlink->friendlink_manage($data);

		ecjia_admin::admin_log($link_name, 'edit', 'friendlink');
		$this->showmessage(RC_Lang::get('friendlink::friend_link.edit_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('friendlink/admin/edit', array('id' => $id))));
	}
	/**
	 * 批量操作
	 */
	public function batch() {
		$this->admin_priv('friendlink_delete');
		...
		if (isset($ids)) {
			if ($type == 'remove') {
				$info = $this->db_friendlink->friendlink_batch(array('link_id' => $ids), 'select', 'link_name, link_logo');
				$this->db_friendlink->friendlink_batch(array('link_id' => $ids), 'delete');
				...
				$this->showmessage(RC_Lang::get('friendlink::friend_link.drop_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('friendlink/admin/init')));
			}
		} else {
			$this->showmessage(RC_Lang::get('friendlink::friendlink.no_select_message'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
	}
	/**
	 * 删除友情链接
	...
	public function remove() {
		...
		$row		= $this->db_friendlink->friendlink_find(array('link_id' => $link_id), 'link_name, link_logo');
		...
		$this->db_friendlink->friendlink_remove($link_id);
	
		ecjia_admin::admin_log($row['link_name'], 'remove', 'friendlink');
		$this->showmessage(RC_Lang::get('friendlink::friend_link.drop_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
	}
	...
	public function remove_logo() {
		...
		$link_id = intval($_GET['id']);
		$link_logo = $this->db_friendlink->friendlink_field(array('link_id' => $link_id), 'link_logo');
		if (!empty($link_logo)) {
			...
			$this->db_friendlink->friendlink_manage($data);
			$this->showmessage(RC_Lang::get('friendlink::friend_link.drop_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('href' => RC_Uri::url('friendlink/admin/edit', array('id' => $link_id))));
		} else {
			$this->showmessage(RC_Lang::get('friendlink::friend_link.drop_fail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
	}
	...
	public function edit_link_name() {
		...
		$link_id   = intval($_POST['pk']);
		$link_name = trim($_POST['value']);
		
		if (empty($link_name)) {
			$this->showmessage(RC_Lang::get('friendlink::friend_link.link_name_empty'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		} else {
			/* 检查链接名称是否重复 */
			if ($this->db_friendlink->friendlink_count(array('link_name' => $link_name)) != 0) {
				$this->showmessage(RC_Lang::get('friendlink::friend_link.link_name_exist'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
				...
				if ($this->db_friendlink->friendlink_manage($data)) {
					ecjia_admin::admin_log($link_name, 'edit', 'friendlink');
					$this->showmessage(RC_Lang::get('friendlink::friend_link.edit_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
				} else {
					$this->showmessage(RC_Lang::get('friendlink::friend_link.edit_fail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
				}
			}
		}
	}
	...
	public function edit_show_order() {		
		...
		/* 检查输入的值是否合法 */
		if (empty($order)) {
			$this->showmessage(RC_Lang::get('friendlink::friend_link.link_order'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		} elseif (!preg_match("/^[0-9]+$/", $order)) {
			$this->showmessage(RC_Lang::get('friendlink::friend_link.enter_int'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		} else {
			$data = array(
				'link_id' 		=> $id,
				'show_order' 	=> $order
			...
			if ($this->db_friendlink->friendlink_manage($data)) {
				$this->showmessage(RC_Lang::get('friendlink::friend_link.edit_success'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('friendlink/admin/init')));
			} else {
				$this->showmessage(RC_Lang::get('friendlink::friend_link.edit_fail'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
		}
	...
	private function get_friendlink_list($args = array()) {
		$db_friendlink = RC_Loader::load_app_model('friendlink_model');
		
	    $where = $filter = array();
	    $filter['keywords'] = empty($_GET['keywords']) ? '' : trim($_GET['keywords']);
	
	    if ($filter['keywords']) {
	        $where[] = "link_name LIKE '%" . mysql_like_quote($filter['keywords']) . "%'";
	    }
	    
	    $count = $db_friendlink->friendlink_count($where);
	    $page = new ecjia_page($count, 10, 5);
	    
	    $option = array(
	    	'where' => $where,
	    	'order'	=> array('show_order' => 'ASC'),
	    	'limit'	=> $page->limit()
	    );
	    $data = $db_friendlink->friendlink_select($option);
